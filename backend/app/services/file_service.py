...
from app.analysis import AIDetectionAnalyzer
...
class FileUploadService:
    ...
    def get_analysis(self, file_id: str) -> dict:
        """
        Perform comprehensive AI detection analysis on an audio file.
        
        Consolidates metadata and pattern analysis to determine if audio
        was likely generated by AI. Uses multiple indicators.
        
        Args:
            file_id (str): The UUID of the file to analyze
            
        Returns:
            dict: Comprehensive analysis with risk score, detected factors, etc.
            
        Raises:
            HTTPException: If file not found or analysis fails
        """
        try:
            # 1. Get metadata and patterns
            metadata_response = self.get_file_metadata(file_id)
            patterns_response = self.get_padding(file_id)
            ...
            # 2. Run analysis
            analysis_result = self.analyzer.analyze(metadata, patterns)
